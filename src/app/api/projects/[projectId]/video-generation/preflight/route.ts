...
import { preflightVideoGeneration } from "@/features/generation/server/generation-service";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await preflightVideoGeneration(user.id, projectId, parsed.data);
    ...
    return serverError("Video generation preflight failed");
  }
}
